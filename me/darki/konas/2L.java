...
extends 2E {
    ...
    public static int c(NonNullList nonNullList) {
        return nonNullList.stream().filter(2L::c).mapToInt(ItemStack::func_190916_E).sum();
    }

    @Override
    public void 5() {
        super.5();
        int n2 = ((IInventoryPlayer)2L.c.player.inventory).getAllInventories().stream().mapToInt(2L::c).sum();
        String string = n2 + " Crystals";
